...
void RegisterAllAppActions(Gui* gui, Renderer* app)
{
	...
	// ----------------------------------------------------
	reg.registerAction({
		"file.open", "Open Map...", "File", "Ctrl+O",
		"Open an existing BSP or supported map file",
		[gui]() { ifd::FileDialog::Instance().Open("OpenMapDialog", "Open BSP Map", "Valve BSP (*.bsp){.bsp},.*"); },
		[]() { return true; }
	});

	reg.registerAction({
		"file.save", "Save Map", "File", "Ctrl+S",
	...
	reg.registerAction({
		"file.save_as", "Save Map As...", "File", "Ctrl+Shift+S",
	...
	reg.registerAction({
		"file.screenshot_overview", "Render Map Overview Screenshot...", "File", "",
		"Open overview screenshot renderer tool",
		[gui]() { if (gui) gui->showOverviewWidget = true; },
		hasMap
	});

	reg.registerAction({
		"file.settings", "Application Settings", "File", "Ctrl+P",
		"Open bspguy preferences and settings",
		[gui]() { if (gui) gui->showSettingsWidget = true; },
		[]() { return true; }
	});

	reg.registerAction({
		"file.exit", "Exit bspguy", "File", "Alt+F4",
		"Close the editor",
		[app]() { if (app && app->window) glfwSetWindowShouldClose(app->window, 1); },
		[]() { return true; }
	});
	...
	reg.registerAction({
		"edit.cut", "Cut Entity", "Edit", "Ctrl+X",
		"Cut selected entity to clipboard",
		[app]() { if (app) app->cutEnt(); },
		[app]() -> bool { return app && !app->pickInfo.selectedEnts.empty(); }
	});

	reg.registerAction({
		"edit.copy", "Copy Entity / Texture", "Edit", "Ctrl+C",
		...
		[app]() -> bool { return app && (!app->pickInfo.selectedEnts.empty() || !app->pickInfo.selectedFaces.empty()); }
	});

	reg.registerAction({
		"edit.paste", "Paste Entity", "Edit", "Ctrl+V",
		"Paste entity from clipboard at camera target",
		[app]() { if (app) app->pasteEnt(false); },
		[app]() -> bool { return app && app->hasCopiedEnt(); }
	});

	reg.registerAction({
		"edit.paste_origin", "Paste at Entity Origin", "Edit", "",
		"Paste copied entity at selected entity's pivot origin",
		[app]() {
			if (!app) return;
			Bsp* map = app->getSelectedMap();
			if (!map || app->pickInfo.selectedEnts.empty()) return;
			vec3 pivot = vec3();
		...
		[app]() -> bool { return app && app->hasCopiedEnt() && !app->pickInfo.selectedEnts.empty(); }
	});

	reg.registerAction({
		"edit.delete", "Delete Entity", "Edit", "Del",
		"Delete selected entities",
		[app]() { if (app) app->deleteEnts(); },
		[app]() -> bool { return app && !app->pickInfo.selectedEnts.empty(); }
	});

	reg.registerAction({
		"edit.select_all", "Select All Entities", "Edit", "Ctrl+A",
	...
	reg.registerAction({
		"edit.deselect_all", "Deselect All", "Edit", "Esc",
	...
	reg.registerAction({
		"edit.select_same_texture", "Select Faces with Same Texture", "Edit", "Ctrl+Alt+A",
		"Select all faces in the map sharing the active face texture",
		[app]() {
			if (!app) return;
			Bsp* map = app->getSelectedMap();
			if (!map || app->pickInfo.selectedFaces.empty()) return;
		...
		[app]() -> bool { return app && !app->pickInfo.selectedFaces.empty(); }
	});

	reg.registerAction({
		"edit.split_face", "Split Face", "Edit", "F",
		"Split face across selected edge vertices",
		[app]() { if (app) app->splitModelFace(); },
		[app]() -> bool { return app && app->pickMode != PICK_OBJECT; }
	});

	reg.registerAction({
		"edit.transform", "Transform Tool", "Edit", "Ctrl+M",
		"Open translation, rotation, scaling transform widget",
		[gui]() { if (gui) gui->showTransformWidget = !gui->showTransformWidget; },
		[app]() -> bool { return app && !app->pickInfo.selectedEnts.empty(); }
	});

	reg.registerAction({
		"edit.keyvalues", "Entity Keyvalues (SmartEdit)", "Edit", "Ctrl+G",
		"Edit keyvalues, flags, and attributes for selected entity",
		[gui]() { if (gui) gui->showKeyvalueWidget = !gui->showKeyvalueWidget; },
		[app]() -> bool { return app && !app->pickInfo.selectedEnts.empty(); }
	});
	...
	// ----------------------------------------------------
	reg.registerAction({
		"view.command_palette", "Command Palette...", "View", "Ctrl+K",
	...
	reg.registerAction({
		"view.ortho_toggle", "Toggle Perspective / Orthographic", "View", "Ctrl+O",
		"Switch between 3D perspective and 2D ortho overview camera",
		[gui]() { if (gui) gui->orthoMode = !gui->orthoMode; },
		[]() { return true; }
	});

	reg.registerAction({
		"view.wireframe_toggle", "Toggle Wireframe Rendering", "View", "Ctrl+W",
	...
	reg.registerAction({
		"view.textures_toggle", "Toggle Textures Rendering", "View", "Ctrl+T",
	...
	reg.registerAction({
		"view.lightmaps_toggle", "Toggle Lightmaps Rendering", "View", "Ctrl+L",
	...
	// ----------------------------------------------------
	reg.registerAction({
		"tools.fix_transparency", "Fix Transparent Rendering", "Tools", "",
	...
	reg.registerAction({
		"tools.fix_face_ranges", "Fix Invalid Model Face Ranges", "Tools", "",
	...
	reg.registerAction({
		"tools.optimize_bsp", "Optimize BSP Geometry", "Tools", "",
	...
	// ----------------------------------------------------
	reg.registerAction({
		"window.limits", "Map Limits & Engine Statistics", "Windows", "F2",
		"Open engine limits and lump usage analyzer",
		[gui]() { if (gui) gui->showLimitsWidget = !gui->showLimitsWidget; },
		hasMap
	});

	reg.registerAction({
		"window.entity_report", "Entity Report Window", "Windows", "F3",
		"Open full searchable entity report table",
		[gui]() { if (gui) gui->showEntityReport = !gui->showEntityReport; },
		hasMap
	});

	reg.registerAction({
		"window.texture_browser", "Texture Browser", "Windows", "F4",
		"Browse embedded and WAD textures",
		[gui]() { if (gui) gui->showTextureBrowser = !gui->showTextureBrowser; },
		hasMap
	});

	reg.registerAction({
		"window.log", "Log & Console Output", "Windows", "F5",
		"Open bspguy output log console",
		[gui]() { if (gui) gui->showLogWidget = !gui->showLogWidget; },
		[]() { return true; }
	});

	reg.registerAction({
		"window.goto", "Go to Coordinates (GOTO)", "Windows", "Ctrl+Shift+G",
		"Teleport camera to specific coordinates or entity index",
		[gui]() { if (gui) gui->showGOTOWidget = !gui->showGOTOWidget; },
		hasMap
	});

	reg.registerAction({
		"window.face_editor", "Face Editor Widget", "Windows", "F6",
		"Open texture alignment, scaling, and face manipulation panel",
		[gui]() { if (gui) gui->showFaceEditWidget = !gui->showFaceEditWidget; },
		hasMap
	});

	reg.registerAction({
		"help.shortcuts", "Help & Keybindings", "Help", "F1",
		"Show help dialogue and keyboard shortcuts list",
		[gui]() { if (gui) gui->showHelpWidget = !gui->showHelpWidget; },
		[]() { return true; }
	});

	reg.registerAction({
		"help.about", "About bspguy", "Help", "",
		"Show application version, build date, and contributor information",
		[gui]() { if (gui) gui->showAboutWidget = !gui->showAboutWidget; },
		[]() { return true; }
	});
}
