...
void RegisterAllAppActions(Gui *gui, Renderer *app)
{
	...
	auto hasMap = [app]() -> bool { return app && app->getSelectedMap() != nullptr; };

	// ----------------------------------------------------
	// FILE ACTIONS
	// ----------------------------------------------------
	reg.registerAction(
		{"file.open", "Open Map...", "File", "Ctrl+O", "Open an existing BSP or supported map file",
		 [gui]() { ifd::FileDialog::Instance().Open("OpenMapDialog", "Open BSP Map", "Valve BSP (*.bsp){.bsp},.*"); },
		 []() { return true; }});

	reg.registerAction({"file.save", "Save Map", "File", "Ctrl+S", "Save current BSP changes",
						[app]()
						{
	...
	reg.registerAction({"file.save_as", "Save Map As...", "File", "Ctrl+Shift+S", "Save current map to a new BSP file",
						[app]()
						{
							Bsp *map = app ? app->getSelectedMap() : nullptr;
							if (map)
								ifd::FileDialog::Instance().Save("SaveMapAsDialog", "Save BSP As",
																 "Valve BSP (*.bsp){.bsp}");
						},
						hasMap});
	...
	reg.registerAction({"file.settings", "Application Settings", "File", "Ctrl+P",
						...
						{
							if (gui)
								gui->showSettingsWidget = true;
						},
						[]() { return true; }});

	reg.registerAction({"file.exit", "Exit bspguy", "File", "Alt+F4", "Close the editor",
						[app]()
						{
							if (app && app->window)
	...
	// EDIT ACTIONS
	// ----------------------------------------------------
	reg.registerAction({"edit.undo", "Undo", "Edit", "Ctrl+Z", "Undo last operation",
						[app]()
						{
							Bsp *map = app ? app->getSelectedMap() : nullptr;
	...
	reg.registerAction({"edit.redo", "Redo", "Edit", "Ctrl+Y", "Redo previously undone operation",
						[app]()
						{
							Bsp *map = app ? app->getSelectedMap() : nullptr;
							if (map && map->getBspRender())
	...
	reg.registerAction({"edit.cut", "Cut Entity", "Edit", "Ctrl+X", "Cut selected entity to clipboard",
						[app]()
						{
							if (app)
								app->cutEnt();
	...
	reg.registerAction({"edit.copy", "Copy Entity / Texture", "Edit", "Ctrl+C", "Copy selected entity or face texture",
						[app, gui]()
						{
							if (!app)
								return;
	...
	reg.registerAction({"edit.paste", "Paste Entity", "Edit", "Ctrl+V", "Paste entity from clipboard at camera target",
						[app]()
						{
							if (app)
								app->pasteEnt(false);
	...
	reg.registerAction({"edit.delete", "Delete Entity", "Edit", "Del", "Delete selected entities",
						[app]()
						{
							if (app)
								app->deleteEnts();
	...
	reg.registerAction({"edit.deselect_all", "Deselect All", "Edit", "Esc", "Deselect all entities and faces",
						[app]()
						{
							if (app)
							{
	...
	reg.registerAction({"edit.split_face", "Split Face", "Edit", "F", "Split face across selected edge vertices",
						[app]()
						{
							if (app)
								app->splitModelFace();
	...
	reg.registerAction({"view.command_palette", "Command Palette...", "View", "Ctrl+K",
						...
						[]() { GuiCommandPalette::getInstance().toggle(); }, []() { return true; }});
	...
	reg.registerAction({"window.limits", "Map Limits & Engine Statistics", "Windows", "F2",
						...
						{
							if (gui)
								gui->showLimitsWidget = !gui->showLimitsWidget;
						},
						hasMap});
	...
	reg.registerAction({"window.texture_browser", "Texture Browser", "Windows", "F4",
						...
						{
							if (gui)
								gui->showTextureBrowser = !gui->showTextureBrowser;
						},
						hasMap});

	reg.registerAction({"window.log", "Log & Console Output", "Windows", "F5", "Open bspguy output log console",
						[gui]()
						{
							if (gui)
								gui->showLogWidget = !gui->showLogWidget;
						},
						[]() { return true; }});
	...
	reg.registerAction({"window.face_editor", "Face Editor Widget", "Windows", "F6",
						...
						{
							if (gui)
								gui->showFaceEditWidget = !gui->showFaceEditWidget;
						},
						hasMap});
	...
	reg.registerAction({"help.shortcuts", "Help & Keybindings", "Help", "F1",
						...
						{
							if (gui)
								gui->showHelpWidget = !gui->showHelpWidget;
						},
						[]() { return true; }});
	...
	reg.registerAction({"help.about", "About bspguy", "Help", "",
						...
						{
							if (gui)
								gui->showAboutWidget = !gui->showAboutWidget;
						},
						[]() { return true; }});
}
