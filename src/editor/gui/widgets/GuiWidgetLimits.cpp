			...
			if (ImGui::BeginTabBar(get_localized_string(LANG_1166).c_str()))
			{
				if (ImGui::BeginTabItem(get_localized_string(LANG_0836).c_str()))
				{
					if (!loadedStats)
					{
						...
						stats.emplace_back(
							calcStat("planes", map->planeCount, map->is_bsp2 ? INT_MAX : MAX_MAP_PLANES, false));
						stats.emplace_back(calcStat("vertexes", map->vertCount, MAX_MAP_VERTS, false));
						stats.emplace_back(calcStat("nodes", map->nodeCount,
													map->is_bsp2 ? INT_MAX : (int)g_limits.maxMapNodes, false));
						stats.emplace_back(
							calcStat("texinfos", map->texinfoCount, map->is_bsp2 ? INT_MAX : MAX_MAP_TEXINFOS, false));
						stats.emplace_back(
							calcStat("faces", map->faceCount, map->is_bsp2 ? INT_MAX : MAX_MAP_FACES, false));
						stats.emplace_back(calcStat("clipnodes", map->clipnodeCount,
													map->is_32bit_clipnodes ? INT_MAX : g_limits.maxMapClipnodes,
													false));
						stats.emplace_back(
							calcStat("leaves", map->leafCount, map->is_bsp2 ? INT_MAX : g_limits.maxMapLeaves, false));
						stats.emplace_back(calcStat("marksurfaces", map->marksurfCount,
													map->is_bsp2 ? INT_MAX : MAX_MAP_MARKSURFS, false));
						stats.emplace_back(calcStat("surfedges", map->surfedgeCount,
													map->is_bsp2 ? INT_MAX : g_limits.maxMapSurfedges, false));
						stats.emplace_back(
							calcStat("edges", map->edgeCount, map->is_bsp2 ? INT_MAX : g_limits.maxMapEdges, false));
						...
						stats.emplace_back(
							calcStat("entities", (unsigned int)map->ents.size(), g_limits.maxMapEnts, false));
						loadedStats = true;
					}
					...
					float midWidth =
						consoleFontLarge->CalcTextSizeA(fontSize * 1.1f, FLT_MAX, FLT_MAX, "    Current / Max    ").x;
...
void Gui::drawLimitTab(Bsp *map, int sortMode)
{
	...
	for (size_t i = 0; i < limitModels[sortMode].size(); i++)
	{
		...
		ImGui::SetCursorPosX(ImGui::GetCursorPosX() + ImGui::GetColumnWidth() -
							 ImGui::CalcTextSize(modelInfos[i].model.c_str()).x - ImGui::GetScrollX() -
							 2 * ImGui::GetStyle().ItemSpacing.x);
		ImGui::Text(modelInfos[i].model.c_str());
		ImGui::NextColumn();

		ImGui::SetCursorPosX(ImGui::GetCursorPosX() + ImGui::GetColumnWidth() -
							 ImGui::CalcTextSize(modelInfos[i].val.c_str()).x - ImGui::GetScrollX() -
							 2 * ImGui::GetStyle().ItemSpacing.x);
		ImGui::Text(modelInfos[i].val.c_str());
		ImGui::NextColumn();

		ImGui::SetCursorPosX(ImGui::GetCursorPosX() + ImGui::GetColumnWidth() -
							 ImGui::CalcTextSize(modelInfos[i].usage.c_str()).x - ImGui::GetScrollX() -
							 2 * ImGui::GetStyle().ItemSpacing.x);
