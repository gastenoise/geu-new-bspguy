 ...
 */
template <typename T> inline T INIReader::Get(const std::string &section, const std::string &name) const
{
	auto const _section = Get(section);
 ...
 * not found
 */
template <typename T> inline T INIReader::Get(const std::string &section, const std::string &name, T &&default_v) const
{
	try
	{
...
template <typename T> inline std::string INIReader::Vec2String(const std::vector<T> &v) const
{
	if (v.empty())
	{
		return "";
...
template <typename T> inline T INIReader::Converter(const std::string &s) const
{
	try
	{
		T v{};
