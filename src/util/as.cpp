...
const char *funcNames[AS_FUNC_COUNT] = {"string GetScriptName()",
...
struct AScript
{
	...
	AScript(std::string file) : path(std::move(file))
	{
		...
		modulename.erase(
			std::remove_if(modulename.begin(), modulename.end(), [](unsigned char c) { return !std::isalnum(c); }),
			modulename.end());
...
void RegisterNatives(asIScriptEngine *engine)
{
	...
	r = engine->RegisterGlobalFunction("int GetSelectedMap()", asFUNCTIONPR(Native_GetSelectedMap, (void), int),
									   asCALL_CDECL);
	print_assert(r >= 0);

	r = engine->RegisterGlobalFunction("int GetSelectedEnt()", asFUNCTIONPR(Native_GetSelectedEnt, (void), int),
									   asCALL_CDECL);
	print_assert(r >= 0);
	...
	r = engine->RegisterGlobalFunction("string GetWorkDir()", asFUNCTIONPR(Native_GetWorkDir, (void), std::string),
									   asCALL_CDECL);
	print_assert(r >= 0);

	r = engine->RegisterGlobalFunction("int CreateEntity(int mapIdx, const string &in)",
									   asFUNCTIONPR(Native_CreateEntity, (int, const std::string &), int),
									   asCALL_CDECL);
	print_assert(r >= 0);

	r = engine->RegisterGlobalFunction("bool RemoveEntity(int entIdx)", asFUNCTIONPR(Native_RemoveEntity, (int), bool),
									   asCALL_CDECL);
	print_assert(r >= 0);

	r = engine->RegisterGlobalFunction(
		"void SetEntKeyVal(int entIdx, const string &in, const string &in)",
	...
	r = engine->RegisterGlobalFunction(
		"int FindEntityByKeyVal(int mapIdx, const string &in, const string &in)",
		asFUNCTIONPR(Native_FindEntityByKeyVal, (int, const std::string &, const std::string &), int), asCALL_CDECL);
	print_assert(r >= 0);

	r = engine->RegisterGlobalFunction("float GetLastFrameTime()", asFUNCTIONPR(Native_GetFrameTime, (void), float),
									   asCALL_CDECL);
	print_assert(r >= 0);
}
...
void RegisterStructs(asIScriptEngine *engine)
{
	...
	r = engine->RegisterObjectBehaviour("vec3", asBEHAVE_CONSTRUCT, "void f()",
										asFUNCTIONPR([](void *memory) { new (memory) vec3(); }, (void *), void),
										asCALL_CDECL_OBJFIRST);
	print_assert(r >= 0);
	r = engine->RegisterObjectBehaviour("vec3", asBEHAVE_CONSTRUCT, "void f(float, float, float)",
										asFUNCTIONPR([](void *memory, float x, float y, float z)
													 { new (memory) vec3(x, y, z); }, (void *, float, float, float),
													 void),
										asCALL_CDECL_OBJFIRST);
	print_assert(r >= 0);
	r = engine->RegisterObjectBehaviour("vec3", asBEHAVE_DESTRUCT, "void f()",
										asFUNCTIONPR([](vec3 *memory) { memory->~vec3(); }, (vec3 *), void),
										asCALL_CDECL_OBJFIRST);
	print_assert(r >= 0);

	r = engine->RegisterObjectMethod("vec3", "void Copy(const vec3 &in)", asMETHOD(vec3, Copy), asCALL_THISCALL);
	print_assert(r >= 0);
	r = engine->RegisterObjectMethod("vec3", "vec3& opAssign(const vec3 &in)", asMETHOD(vec3, CopyAssign),
									 asCALL_THISCALL);
	print_assert(r >= 0);
	r = engine->RegisterObjectMethod("vec3", "vec3 opNeg()", asMETHOD(vec3, invert), asCALL_THISCALL);
	print_assert(r >= 0);

	r = engine->RegisterObjectMethod("vec3", "void opSubAssign(const vec3 &in)",
									 asMETHODPR(vec3, operator-=, (const vec3 &), void), asCALL_THISCALL);
	print_assert(r >= 0);
	r = engine->RegisterObjectMethod("vec3", "void opAddAssign(const vec3 &in)",
									 asMETHODPR(vec3, operator+=, (const vec3 &), void), asCALL_THISCALL);
	print_assert(r >= 0);
	r = engine->RegisterObjectMethod("vec3", "void opMulAssign(const vec3 &in)",
									 asMETHODPR(vec3, operator*=, (const vec3 &), void), asCALL_THISCALL);
	print_assert(r >= 0);
	r = engine->RegisterObjectMethod("vec3", "void opDivAssign(const vec3 &in)",
									 asMETHODPR(vec3, operator/=, (const vec3 &), void), asCALL_THISCALL);
	print_assert(r >= 0);

	r = engine->RegisterObjectMethod("vec3", "void opSubAssign(float)", asMETHODPR(vec3, operator-=, (float), void),
									 asCALL_THISCALL);
	print_assert(r >= 0);
	r = engine->RegisterObjectMethod("vec3", "void opAddAssign(float)", asMETHODPR(vec3, operator+=, (float), void),
									 asCALL_THISCALL);
	print_assert(r >= 0);
	r = engine->RegisterObjectMethod("vec3", "void opMulAssign(float)", asMETHODPR(vec3, operator*=, (float), void),
									 asCALL_THISCALL);
	print_assert(r >= 0);
	r = engine->RegisterObjectMethod("vec3", "void opDivAssign(float)", asMETHODPR(vec3, operator/=, (float), void),
									 asCALL_THISCALL);
	print_assert(r >= 0);

	r = engine->RegisterObjectMethod("vec3", "float& opIndex(int)", asMETHODPR(vec3, operator[], (size_t), float &),
									 asCALL_THISCALL);
	print_assert(r >= 0);
	r = engine->RegisterObjectMethod("vec3", "float opIndex(int) const",
									 asMETHODPR(vec3, operator[], (size_t) const, float), asCALL_THISCALL);
	print_assert(r >= 0);
	...
	print_assert(r >= 0);
	r = engine->RegisterObjectMethod("vec3", "vec3 opDiv(float) const",
									 asMETHODPR(vec3, operator/, (float) const, vec3), asCALL_THISCALL);
	print_assert(r >= 0);

	r = engine->RegisterGlobalFunction("vec3 opMul_r(float)", asFUNCTIONPR(operator*, (float, const vec3 &), vec3),
									   asCALL_CDECL);
	print_assert(r >= 0);
}
...
template <typename T> int ExecuteFunction(AScript *script, AS_FUNCS funcEnum, T &resultVar)
{
