...
struct COLOR3
{
	...
	bool operator<(const COLOR3 &other) const { return r < other.r && g < other.g && b < other.b; }
	bool operator>(const COLOR3 &other) const { return other < *this; }
...
struct vec3
{
	...
	vec3(const vec3 &other) { Copy(other); }
	...
	bool operator<(const vec3 &b) const { return std::tie(x, y, z) < std::tie(b.x, b.y, b.z); }
	...
	bool IsZero() const;
	vec3 invert();
	std::string toKeyvalueString(bool truncate = false, const std::string &suffix_x = " ",
								 const std::string &suffix_y = " ", const std::string &suffix_z = "");
	std::string toString();
