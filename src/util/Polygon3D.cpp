...
{
	return sizeof(Polygon3D) + sizeof(vec3) * verts.size() + sizeof(vec2) * localVerts.size() +
...
std::vector<std::vector<vec3>> Polygon3D::cut(Line2D cutLine)
{
	...
	for (size_t i = 0; i < localVerts.size(); i++)
	{
		...
		if (edge.doesIntersect(cutLine))
		{
			...
			if (!vec3Equal(worldPos, verts[i], SAME_VERT_EPSILON) &&
				!vec3Equal(worldPos, verts[next], SAME_VERT_EPSILON))
			{
