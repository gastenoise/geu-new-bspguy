...
int merge_maps()
{
	...
	MergeResult result = merger.merge(maps, output_name, g_cmdLine.hasOption("-noripent"),
									  g_cmdLine.hasOption("-noscript"), g_cmdLine.hasOption("-nostyles"), overlapGaps);
...
int transform()
{
	...
	if (map->bsp_valid)
	{
		...
		if (g_cmdLine.hasOptionVector("-move"))
		{
			...
			print_log(fmt::format(fmt::runtime(get_localized_string("APPLY_OFFSET_STR")), move.x, move.y, move.z));
...
int modent()
{
	...
	for (const auto &file : files)
	{
		...
		if (doDelete)
		{
			auto it = std::remove_if(map->ents.begin(), map->ents.end(),
									 [&](Entity *ent)
									 {
										 if (ent->isWorldSpawn())
											 return false;
...
void print_help(const std::string &command)
{
	if (command == "merge")
	{
		print_log(PRINT_RED | PRINT_GREEN | PRINT_INTENSITY, "{}",
				  ...
				  "  -overlapgap \"X,Y,Z;X2,Y2,Z2;...\" : Overlap positions for each map (default "
				  "\"0,0,0;0,0,512;0,0,1024;...\").\n"
				  "  -v\n"
...
void make_minidump(EXCEPTION_POINTERS *e)
{
	...
	GetSystemTime(&t);
	createDir("./crashes");
	std::string name = fmt::format("./crashes/{}_{:04}{:02}{:02}_{:02}{:02}{:02}{:02}.dmp", "bspguy", t.wYear, t.wMonth,
								   t.wDay, t.wHour, t.wMinute, t.wSecond, crashdumps);
	...
	pMiniDumpWriteDump(GetCurrentProcess(), GetCurrentProcessId(), hFile, MINIDUMP_TYPE(MiniDumpNormal),
					   e ? &exceptionInfo : NULL, NULL, NULL);
...
int main(int argc, char *argv[])
{
	...
	try
	{
		...
		if constexpr (sizeof(vec4) != 16 || sizeof(vec3) != 12 || sizeof(vec2) != 8 || sizeof(COLOR3) != 3 ||
					  sizeof(COLOR4) != 4)
		{
		...
		if (g_cmdLine.command == "screenshot")
		{
			...
			if (g_cmdLine.hasOption("-out"))
			{
				...
				{
					fixupPath(make_screenshot_dir, FIXUPPATH_SLASH::FIXUPPATH_SLASH_SKIP,
		...
		{
			Bsp *tmpBsp = new Bsp(g_cmdLine.bspfile);
			tmpBsp->ExportEmbeddedWad(g_cmdLine.hasOption("-o") ? g_cmdLine.getOption("-o")
																: g_cmdLine.bspfile + ".wad");
			delete tmpBsp;
			retval = 0;
		}
