...
ShaderProgram::ShaderProgram(const char *vshaderSource, const char *fshaderSource)
	: elementSize(0), modelViewID(-1), modelViewProjID(-1), ID(0xFFFFFFFF), vposID(0), vcolorID(0), vtexID(0),
	  modelViewMat(NULL), modelViewProjMat(NULL), attributesBound(false), updMatGlobalId(9999999)
{
...
void ShaderProgram::setVertexAttributeNames(const char *posAtt, const char *colorAtt, const char *texAtt, int attFlags)
{
	if (posAtt)
	{
		...
		if (vposID == -1)
			print_log(get_localized_string(LANG_0965), posAtt);
	}
	if (colorAtt)
	{
		vcolorID = glGetAttribLocation(ID, colorAtt);
		if (vcolorID == -1)
			print_log(get_localized_string(LANG_0966), colorAtt);
	}
	if (texAtt)
	{
		vtexID = glGetAttribLocation(ID, texAtt);
		if (vtexID == -1)
			print_log(get_localized_string(LANG_0967), texAtt);
	}
