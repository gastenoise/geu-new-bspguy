...
MergeResult BspMerger::merge(std::vector<Bsp *> maps, const std::string &output_name, bool noripent, bool noscript,
							 bool nomergestyles, std::vector<vec3> overlapGaps)
{
	MergeResult result;
	result.fpath = "";
	result.map = NULL;
	...
	if (!skipLightStyles)
	{
		...
		for (int i = 0; i < maps[0]->faceCount; i++)
		{
			for (int s = 0; s < MAX_LIGHTMAPS; s++)
			{
				...
				if (style < 255 && style >= start_toggle_lightstyle && !usage_lightstyles.count(style))
				{
			...
			for (int i = 0; i < mapB->faceCount; i++)
			{
				for (int s = 0; s < MAX_LIGHTMAPS; s++)
				{
					...
					if (style < 255 && style >= start_toggle_lightstyle && usage_lightstyles.count(style))
					{
						...
						{ /*
							 print_log(PRINT_GREEN,"REMAP {} TO {}\n", mapB.faces[i].nStyles[s],
							 remap_light_styles[style]);*/
			...
			for (size_t i = 0; i < mapB->ents.size(); i++)
			{
				...
				if (mapent->keyvalues["classname"].find("light") != std::string::npos)
				{
					if (mapent->hasKey("style"))
					{
						...
						if (style < 255 && style >= start_toggle_lightstyle &&
							remap_light_styles.find((unsigned char)style) != remap_light_styles.end())
						{
							remapped_lightents++;
	...
	for (size_t z = 0; z < blocks.size(); z++)
	{
		for (size_t y = 0; y < blocks[z].size(); y++)
		{
			for (size_t x = 0; x < blocks[z][y].size(); x++)
			{
				...
				if (std::fabs(block.offset.x) >= EPSILON || std::fabs(block.offset.y) >= EPSILON ||
					std::fabs(block.offset.z) >= EPSILON)
				{
					print_log("    Apply offset ({:6.0f}, {:6.0f}, {:6.0f}) to {}\n", block.offset.x, block.offset.y,
							  block.offset.z, block.map->bsp_name.c_str());
...
std::vector<std::vector<std::vector<MAPBLOCK>>> BspMerger::separate(std::vector<Bsp *> &maps, MergeResult &result,
...
void BspMerger::update_map_series_entity_logic(Bsp *mergedMap, std::vector<MAPBLOCK> &sourceMaps,
											   std::vector<Bsp *> &mapOrder, const std::string &output_name,
											   const std::string &firstMapName, bool noscript)
{
	...
	for (int i = 0; i < originalEntCount; i++)
	{
		...
		if (noscript && !isInFirstMap)
		{
			...
			if (starts_with(cname, "monster_") && cname.rfind("_dead") != cname.size() - 5)
			{
				...
				ent->clearEmptyKeyvalues(); // things like the model keyvalue will break the monster if it's set but
											// empty
		...
		if (cname == "trigger_changelevel")
		{
			...
			if (noscript)
			{
				...
				// ".ent_create trigger_changevalue
				// "targetname:kill_me:target:!activator:m_iszValueName:targetname:m_iszNewValue:bee_gun:message:kill_me2"
...
bool BspMerger::merge(Bsp &mapA, Bsp &mapB, bool modelMerge)
{
	...
	if (separationPlane.nType == -1 && !modelMerge)
	{
		...
		separationPlane = getSeparatePlane(mapA.models[0].nMins, mapA.models[0].nMaxs, mapB.models[0].nMins,
										   mapB.models[0].nMaxs, true);
	...
	for (int i = 0; i < mapB.planeCount; i++)
	{
		...
		for (int k = 0; k < mapA.planeCount; k++)
		{
			if (std::fabs(mapB.planes[i].fDist - mapA.planes[k].fDist) < EPSILON &&
				mapB.planes[i].nType == mapA.planes[k].nType && mapB.planes[i].vNormal == mapA.planes[k].vNormal)
			{
				isUnique = false;
...
void BspMerger::merge_textures(Bsp &mapA, Bsp &mapB)
{
	...
	unsigned int maxMipTexDataSize =
		mapA.bsp_header.lump[LUMP_TEXTURES].nLength + mapB.bsp_header.lump[LUMP_TEXTURES].nLength;
...
void BspMerger::merge_texinfo(Bsp &mapA, Bsp &mapB)
{
	...
	for (int i = 0; i < mapB.texinfoCount; i++)
	{
		...
		{
			if (info.iMiptex == mapA.texinfos[k].iMiptex && info.nFlags == mapA.texinfos[k].nFlags &&
				std::fabs(info.shiftS - mapA.texinfos[k].shiftS) < EPSILON &&
				std::fabs(info.shiftT - mapA.texinfos[k].shiftT) < EPSILON && info.vS == mapA.texinfos[k].vS &&
...
void BspMerger::merge_models(Bsp &mapA, Bsp &mapB)
{
	...
	for (int i = 1; i < mapB.modelCount; i++)
	{
		...
		if (model.iHeadnodes[0] >= 0)
			model.iHeadnodes[0] +=
				thisNodeCount; // already includes new head nodes (merge_nodes comes after create_merge_headnodes)
...
void BspMerger::merge_vis(Bsp &mapA, Bsp &mapB)
{
	...
	decompress_vis_lump(&mapA, allLeaves, mapA.visdata, decompressedVis, thisWorldLeafCount, thisVisLeaves,
						totalVisLeaves, mapA.bsp_header.lump[LUMP_LEAVES].nLength, mapA.visDataLength);
	...
	decompress_vis_lump(&mapB, allLeaves + thisWorldLeafCount, mapB.visdata, decompressedOtherVis, otherWorldLeafCount,
						otherLeafCount, totalVisLeaves, mapB.bsp_header.lump[LUMP_LEAVES].nLength, mapB.visDataLength);
	...
	int newVisLen = CompressAll(allLeaves, decompressedVis, compressedVis, totalVisLeaves, mergedWorldLeafCount,
								decompressedVisSize, thisWorldLeafCount + otherWorldLeafCount);
...
void BspMerger::create_merge_headnodes(Bsp &mapA, Bsp &mapB, BSPPLANE separationPlane)
{
	...
	bool swapNodeChildren =
		separationPlane.vNormal.x < 0 || separationPlane.vNormal.y < 0 || separationPlane.vNormal.z < 0;
	if (swapNodeChildren)
