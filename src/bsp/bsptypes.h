...
static const char *g_lump_names[HEADER_LUMPS] = {"ENTITIES", "PLANES",		 "TEXTURES", "VERTICES",  "VISIBILITY",
...
struct BSPPLANE
{
	...
	BSPPLANE operator-() const { return BSPPLANE(-vNormal, -fDist, nType); }
};
...
struct BSPFACE32
{
	...
	BSPFACE32()
	{
		...
		nLightmapOffset = -1;
		nStyles[0] = nStyles[1] = nStyles[2] = nStyles[3] = 255;
	}
};
...
struct BSPLEAF32
{
	...
	BSPLEAF32()
	{
		...
		iFirstMarkSurface = 0;
		nMarkSurfaces = 0;
		nAmbientLevels[0] = nAmbientLevels[1] = nAmbientLevels[2] = nAmbientLevels[3] = 0;
	}
	bool isEmpty();
};
...
struct BSPMODEL
{
	...
	BSPMODEL()
	{
		...
		iHeadnodes[0] = iHeadnodes[1] = iHeadnodes[2] = iHeadnodes[3] = -1;
...
struct BSPNODE32
{
	...
	BSPNODE32(int plane, const std::array<int, 2> &childs, const vec3 &mins, const vec3 &maxs, int firstface, int faces)
		: iPlane(plane), nMins(mins), nMaxs(maxs), iFirstFace(firstface), nFaces(faces)
	{
...
struct BSPNODE16
{
	...
	short iChildren[2]; // If < 0, then bitwise inverse indices into Leafs. For example ~(-1) == 0  // otherwise indices
						// into Nodes
	short nMins[3], nMaxs[3];		  // Defines bounding box
