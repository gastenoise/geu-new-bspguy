...
class Bsp
{
  public:
	...
	void get_last_clipnode(int nodeIdx, int &node, int &count,
						   int last_node = -1); // get leaf index from world position
	...
	bool getModelPlaneIntersectVerts(int modelIdx, const std::vector<int> &planes,
	...
	STRUCTCOUNT remove_unused_model_structures(unsigned int target = CLEAN_LIGHTMAP | CLEAN_PLANES | CLEAN_NODES |
																	 CLEAN_CLIPNODES | CLEAN_LEAVES |
																	 CLEAN_MARKSURFACES | CLEAN_FACES |
																	 CLEAN_SURFEDGES | CLEAN_TEXINFOS | CLEAN_EDGES |
																	 CLEAN_VERTICES | CLEAN_TEXTURES | CLEAN_VISDATA |
																	 CLEAN_MODELS);
	...
	void delete_oob_clipnodes(int iNode, int *parentBranch, std::vector<BSPPLANE> &clipOrder, int oobFlags,
							  bool *oobHistory, bool isFirstPass, int &removedNodes);

	void delete_oob_nodes(int iNode, int *parentBranch, std::vector<BSPPLANE> &clipOrder, int oobFlags,
						  bool *oobHistory, bool isFirstPass, int &removedNodes);
	...
	int delete_box_clipnodes_fast(int iNode, std::vector<BSPPLANE> &clipOrder, vec3 clipMins, vec3 clipMaxs,
								  int redirect = CONTENTS_SOLID);
	int delete_box_nodes_fast(int iNode, vec3 clipMins, vec3 clipMaxs, int redirect = CONTENTS_SOLID);

	void delete_box_clipnodes(int iNode, int *parentBranch, std::vector<BSPPLANE> &clipOrder, vec3 clipMins,
							  vec3 clipMaxs, bool *oobHistory, bool isFirstPass, int &removedNodes,
							  int redirect = CONTENTS_SOLID);
	void delete_box_nodes(int iNode, int *parentBranch, std::vector<BSPPLANE> &clipOrder, vec3 clipMins, vec3 clipMaxs,
						  bool *oobHistory, bool isFirstPass, int &removedNodes, int redirect = CONTENTS_SOLID);
	...
	// allowWad:true = texture coordinates will be scaled even if the the texture is from a WAD and must be scaled
	// separately returns true if was downscaled
	...
	int create_node_box(const vec3 &mins, const vec3 &maxs, BSPMODEL *targetModel, bool empty = false,
						int leafIdx = -1);
	int create_clipnode_box(const vec3 &mins, const vec3 &maxs, BSPMODEL *targetModel, int targetHull = 0,
							bool skipEmpty = false, bool empty = false);
	...
	void copy_bsp_model(int modelIdx, Bsp *targetMap, STRUCTREMAP &remap, STRUCTUSAGE &usage,
						std::vector<BSPPLANE> &newPlanes, std::vector<vec3> &newVerts, std::vector<BSPEDGE32> &newEdges,
						std::vector<int> &newSurfedges, std::vector<BSPTEXTUREINFO> &newTexinfo,
						std::vector<BSPFACE32> &newFaces, std::vector<COLOR3> &newLightmaps,
						std::vector<BSPNODE32> &newNodes, std::vector<BSPCLIPNODE32> &newClipnodes,
						std::vector<WADTEX> &newTextures, std::vector<BSPLEAF32> &newLeafs,
						std::vector<int> &newMarkSurfs, bool forExport = false);
	...
	// if the face's texinfo is not unique, a new one is created and returned. Otherwise, it's current texinfo is
	// returned
	...
	unsigned int remove_unused_visdata(BSPLEAF32 *oldLeaves, int oldWorldLeaves,
									   int oldLeavesMemSize); // called after removing unused leaves
	unsigned int remove_unused_textures(std::vector<bool> &usedTextures, std::vector<int> &remappedIndexes,
