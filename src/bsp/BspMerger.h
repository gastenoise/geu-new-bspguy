...
struct MAPBLOCK
{
	...
	bool intersects(MAPBLOCK &other)
	{
		return (mins.x <= other.maxs.x && maxs.x >= other.mins.x) &&
			   (mins.y <= other.maxs.y && maxs.y >= other.mins.y) && (mins.z <= other.maxs.z && maxs.y >= other.mins.z);
	}
};
...
class BspMerger
{
  public:
	...
	MergeResult merge(std::vector<Bsp *> maps, const std::string &output_name, bool noripent, bool noscript,
	...
	std::vector<std::vector<std::vector<MAPBLOCK>>> separate(std::vector<Bsp *> &maps, MergeResult &result,
	...
	void update_map_series_entity_logic(Bsp *mergedMap, std::vector<MAPBLOCK> &sourceMaps, std::vector<Bsp *> &mapOrder,
										const std::string &output_name, const std::string &firstMapName, bool noscript);
