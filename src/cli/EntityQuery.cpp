...
class NotNode : public QueryNode
{
  public:
	...
	bool evaluate(Entity *ent) override { return !child->evaluate(ent); }
};
...
void EntityQuery::tokenize(const std::string &q)
{
	for (size_t i = 0; i < q.size();)
	{
		...
		{
			std::string s;
			while (i < q.size() && !isspace((unsigned char)q[i]) && q[i] != '(' && q[i] != ')' && q[i] != '=' &&
				   q[i] != '!' && q[i] != '"')
			{
				s += q[i++];
			}
			...
			{
				std::string upperS = s;
				std::transform(upperS.begin(), upperS.end(), upperS.begin(),
							   [](unsigned char c) { return (unsigned char)std::toupper(c); });

				if (upperS == "AND")
					tokens.push_back({Token::AND, s});
				else if (upperS == "OR")
					tokens.push_back({Token::OR, s});
				else if (upperS == "NOT")
					tokens.push_back({Token::NOT, s});
				else
					tokens.push_back({Token::KEYVALUE, s});
			}
		}
