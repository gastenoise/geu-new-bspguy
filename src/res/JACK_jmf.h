...
class JackWriter
{
  public:
	...
	template <typename T> void write(const T &value) { file.write((const char *)&value, sizeof(T)); }
	...
	bool is_open() const { return file && file.is_open(); }
...
class JackReader
{
  public:
	...
	bool readKeyVal(std::string &key, std::string &val) { return readLenStr(key) && readLenStr(val); }
