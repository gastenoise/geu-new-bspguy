	...
	for (auto &s : sides)
	{
		...
		for (unsigned int i = 1; i + 1 < poly.size(); i++)
		{
			for (int j = 0; j < 3; j++)
			{
				...
				vert.pos = poly[idx];
				vert.u = s.vecs[0].x * vert.pos.x + s.vecs[0].y * vert.pos.y + s.vecs[0].z * vert.pos.z + s.vecs[0].w;
				vert.v = s.vecs[1].x * vert.pos.x + s.vecs[1].y * vert.pos.y + s.vecs[1].z * vert.pos.z + s.vecs[1].w;
...
void CSMFile::printInfo()
{
	...
	print_log("  Sides: {}\n", sides.size());
	print_log("  Points: {}\n", points.size());
	print_log("  Bounds: ({:.2f},{:.2f},{:.2f}) to ({:.2f},{:.2f},{:.2f})\n", header.model_mins.x, header.model_mins.y,
			  header.model_mins.z, header.model_maxs.x, header.model_maxs.y, header.model_maxs.z);
