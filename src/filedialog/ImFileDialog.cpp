...
bool PathBox(const char *label, std::filesystem::path &path, char *pathBuffer, int len, ImVec2 size_arg)
{
	...
	if (!(*state & 0b001))
	{
		...
		for (size_t i = 0; i < btnList.size(); i++)
		{
			...
			if (!isFirstElement)
			{
				ImGui::ArrowButtonEx(get_localized_string(LANG_0930).c_str(), ImGuiDir_Right,
	...
	else
	{
		...
		if (ImGui::InputTextEx(get_localized_string(LANG_0931).c_str(), "", pathBuffer, 1024, size_arg,
...
void FileDialog::m_renderContent()
{
	...
	if (m_zoom == 1.0f)
	{
		if (ImGui::BeginTable(get_localized_string(LANG_0932).c_str(), 3, ImGuiTableFlags_Sortable,
							  ImVec2(0, -FLT_MIN)))
		{
			// header
			ImGui::TableSetupColumn(get_localized_string(LANG_0933).c_str(), ImGuiTableColumnFlags_WidthFixed, -1.0f,
									0);
			ImGui::TableSetupColumn(get_localized_string(LANG_0934).c_str(),
									ImGuiTableColumnFlags_WidthFixed | ImGuiTableColumnFlags_NoResize, 0.0f, 1);
			ImGui::TableSetupColumn(get_localized_string(LANG_0935).c_str(),
			...
			for (const auto &entry : m_content)
			{
				const std::string filename =
					entry.Path.filename().string().empty() ? entry.Path.string() : entry.Path.filename().string();
				...
				if (tm != nullptr)
					ImGui::Text(fmt::format("{}/{}/{} {:2d}:{:2d}", tm->tm_mon + 1, tm->tm_mday, 1900 + tm->tm_year,
...
void FileDialog::m_renderPopups()
{
	...
	if (ImGui::BeginPopupModal(get_localized_string(LANG_1132).c_str()))
	{
		...
		else
		{
			...
			ImGui::TextWrapped(
				fmt::format(fmt::runtime(get_localized_string(LANG_0929)), data.Path.filename().string().c_str())
					.c_str());
	...
	if (ImGui::BeginPopupModal(get_localized_string(LANG_1133).c_str()))
	{
		...
		ImGui::InputText(get_localized_string(LANG_0944).c_str(), m_newEntryBuffer,
	...
	{
		ImGui::PushItemWidth(250.0f);
		ImGui::InputText(get_localized_string(LANG_1135).c_str(), m_newEntryBuffer,
						 1024); // TODO: remove hardcoded literals
		ImGui::PopItemWidth();
...
void FileDialog::m_renderFileDialog()
{
	...
	if (ImGui::ArrowButtonEx(get_localized_string(LANG_0946).c_str(), ImGuiDir_Left,
							 ImVec2(GUI_ELEMENT_SIZE, GUI_ELEMENT_SIZE),
	...
	if (noForwardHistory)
		ImGui::PushStyleVar(ImGuiStyleVar_Alpha, ImGui::GetStyle().Alpha * 0.5f);
	if (ImGui::ArrowButtonEx(get_localized_string(LANG_0947).c_str(), ImGuiDir_Right,
							 ImVec2(GUI_ELEMENT_SIZE, GUI_ELEMENT_SIZE),
	...
	if (ImGui::InputTextEx(get_localized_string(LANG_0948).c_str(), get_localized_string(LANG_0949).c_str(),
						   m_searchBuffer, 128, ImVec2(-FLT_MIN, GUI_ELEMENT_SIZE), 0)) // TODO: no hardcoded literals
		m_setDirectory(m_currentDirectory, false);										// refresh

	/***** CONTENT *****/
	float bottomBarHeight =
		(GImGui->FontSize + ImGui::GetStyle().FramePadding.y + ImGui::GetStyle().ItemSpacing.y * 2.0f) * 2;
	if (ImGui::BeginTable(get_localized_string(LANG_0950).c_str(), 2, ImGuiTableFlags_Resizable,
						  ImVec2(0, -bottomBarHeight)))
	{
		...
		ImGui::TableSetColumnIndex(1);
		ImGui::BeginChild(get_localized_string(LANG_0954).c_str(), ImVec2(0, -bottomBarHeight), 0,
	...
	ImGui::SameLine();
	if (ImGui::InputTextEx(get_localized_string(LANG_0956).c_str(), get_localized_string(LANG_0957).c_str(),
						   m_inputTextbox, 1024, ImVec2((m_type != IFD_DIALOG_DIRECTORY) ? -250.0f : -FLT_MIN, 0),
						   ImGuiInputTextFlags_EnterReturnsTrue))
	{
